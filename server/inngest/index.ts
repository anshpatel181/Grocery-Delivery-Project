...
const LOW_STOCK_THRESHOLD = 10; // we will send email when stock of any product goes under 10
...
const checkLowStock = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (!product || product.stock === null || product.stock >= LOW_STOCK_THRESHOLD) return { skipped: true, stock: product?.stock }
...
const sendMonthlyOffers = inngest.createFunction({
  ...
  if (users.length === 0 || deals.length === 0) return { skipped: true, message: "No users or deals" }
  ...
  for (let i = 0; i < users.length; i += batchSize) {
    ...
    await step.run(`send-offers-batch-${i}`, async () => {
      for (const u of batch) {
        await sendEmail({
          to: u.email, subject: `Fresh Picks Just For You!`, body: `<div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 520px; margin: auto; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden;">
...
const autoAssignRider = inngest.createFunction({
  ...
  // wait 5 minutes before attempting assignment, so whenever this event is triggered then it will run after 5 minutes of sleep. like when the order is created that means a customer has just ordered something then at that time we will store that order in db and trigger this event which will sleep for 5 minutes first and then run following code which assigns the order to available delivery partener which is currently not delivering anything
  ...
  const result = await step.run("assign-rider", async () => {
    ...
    if(order.deliveryPartnerId) return {skipped: true, reason: "Delivery Partener already assigned"}
    if(["Cancelled", "Delivered"].includes(order.status)) return {skipped: true, reason: `Order ${order.status}`}

    const busyOrders = await prisma.order.findMany({where: {status: {in: ["Assigned", "Packed", "Out for Delivery"]}, deliveryPartnerId: {not: null}}, select: {deliveryPartnerId: true}})
    ...
    if(!availableRider) return {skipped: true, message: "No riders available"}
    ...
    await prisma.order.update({where: {id: orderId}, data: {deliveryPartnerId: availableRider.id, status: "Assigned", statusHistory: history, deliveryOtp: OTP}})
