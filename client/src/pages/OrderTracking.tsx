...
import { ArrowLeftIcon, DotIcon, MapPinIcon, PhoneIcon } from "lucide-react";
...
export const OrderTracking = () => {
  ...
  // Live location at every 10 seconds
  useEffect(() => {
    ...
    return () => clearInterval(interval) // clearInterval is neccessary because when component unmounts we have to stop the interval and that's why we are clearing timed events that are running when we unmount this component. because timer events are browser api's specifically web api's which are handled by browser not react itself so when component unmounts react is not able to automatically stop currently running events
  }, [id, order?.status])
              ...
              <div className="bg-white rounded-2xl p-5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-app-text-light capitalize">{order.deliveryPartner.vehicleType} <DotIcon className="size-3" /> Delivery Partner</p>
                  </div>
                </div>
